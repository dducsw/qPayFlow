...
`Receive Payment -> Validate -> Reserve Balance -> Fraud Check -> Process Payment -> Settlement -> Notification`
...
2. **Go Concurrency Primitives & Lifecycle Management**
...
5. **Saga Pattern & Eventual Consistency**
   - Implement **Saga choreography** via Kafka. Workflow steps through: Payment -> Reserve -> Fraud -> Process. Failures (e.g., Fraud Rejected) trigger **Compensation** flows (Release balance).
6. **Kafka Architecture & Partitioning**
   - Maintain **Ordering** by using `account_id` as partition key. Explore Consumer Groups, backpressure, and consumer lag.
7. **Retry Architecture & Dead Letter Queue (DLQ)**
...
9. **Leader Election & Background Jobs**
...
12. **Change Data Capture (CDC) with Debezium vs Outbox Polling**
...
13. **Event Sourcing & CQRS (Command Query Responsibility Segregation)**
...
14. **Field-Level Encryption & Tokenization (PCI-DSS & Security)**
...
15. **Distributed Rate Limiting & Load Shedding**
...
16. **Distributed Context Propagation over Asynchronous Boundaries**
...
18. **Site Reliability Engineering (SRE) Principles**
...
* **Phase 0 — Foundation (1-2 days):** Go project structure, Docker, PostgreSQL, Redis, Kafka, config, logging.
* **Phase 1 — Payment MVP (3-5 days):** Setup DB, build basic payment flow without Kafka first (correct business logic).
